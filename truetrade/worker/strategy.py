"""Explicit demo baseline using the existing causal features, not a trained PPO.
...
class WorkerSettings:
    ...
    def __post_init__(self):
        if self.mode not in {"paper", "demo"}:
            raise ValueError("Worker supports paper/demo only; baseline is not approved for live")
        if self.strategy != "breakout_demo":
            raise ValueError("Only explicitly named breakout_demo strategy is implemented")
        ...
        if not self.risk.is_finite() or not 0 < self.risk <= Decimal(".005"):
            raise ValueError("Demo baseline risk must be positive and at most 0.5 percent")

    @classmethod
    def from_env(cls):
        return cls(mode=os.getenv("MT5_MODE", "paper"), strategy=os.getenv("MT5_STRATEGY", "breakout_demo"),
